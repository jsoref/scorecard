...
func collectDockerfilePinning(c *checker.CheckRequest, r *checker.PinningDependenciesData) error {
	...
	for i := range r.Dependencies {
		rr := &r.Dependencies[i]
		if !*rr.Pinned {
			remediation := remediation.CreateDockerfilePinningRemediation(rr, remediation.CraneDigester{})
			rr.Remediation = remediation
		}
	}
	return nil
}
...
func collectGitHubActionsWorkflowPinning(c *checker.CheckRequest, r *checker.PinningDependenciesData) error {
	...
	for i := range r.Dependencies {
		rr := &r.Dependencies[i]
		if !*rr.Pinned {
			remediation := remediationMetadata.CreateWorkflowPinningRemediation(rr.Location.Path)
			rr.Remediation = remediation
		}
	}
	return nil
